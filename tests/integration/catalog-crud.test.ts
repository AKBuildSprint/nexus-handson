...
import { consoleRequest, resetCatalog, SIMPLE_CORE } from '../support/catalog-test-env';
...
describe('catalog CRUD and stable errors', () => {
  it('creates, lists, opens, updates, and rejects stale revisions', async () => {
    ...
    const list = await consoleRequest('/api/console/products?q=field&status=active');
    expect(await list.json()).toMatchObject({ products: [{ id: mutation.product.id, slug: 'field-notes', enabledVariantCount: null }] });

    const detail = await consoleRequest('/api/console/products/by-slug/field-notes');
    ...
    const updated = await consoleRequest(`/api/console/products/${mutation.product.id}`, {
    ...
    const stale = await consoleRequest(`/api/console/products/${mutation.product.id}`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json', 'If-Match': '"1"' }, body: JSON.stringify(updateBody),
    });
  ...
  it('uses the stable JSON envelope for unknown API routes', async () => {
    const response = await consoleRequest('/api/console/products/missing/extra');
