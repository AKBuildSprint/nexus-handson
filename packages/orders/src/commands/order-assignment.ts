...
/** One replan is enough to absorb a single lost prestate race; a second stale read is reported as a conflict. */
const ASSIGNMENT_REPLAN_LIMIT = 1;
...
export async function assignOrder(input: {
  ...
  for (let replans = 0; ; replans += 1) {
    ...
    try {
      ...
      await assertCurrentOwner(input.database, input.identity);
      const plannedFromStalePrestate = (currentNow?.assignee_user_id ?? null) !== (current?.assignee_user_id ?? null)
        || (currentNow?.history_id ?? null) !== (current?.history_id ?? null);
      if (plannedFromStalePrestate) {
        if (replans < ASSIGNMENT_REPLAN_LIMIT) continue;
        stateConflict();
      }
