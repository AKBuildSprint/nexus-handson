...
Tokens: [`src/styles/design-tokens.css`](src/styles/design-tokens.css). Destinations are Products, Orders, and owner-only Third-party logs. Import `@nexus/catalog/*` only — do not depend on `@nexus/orders`. Order UI types stay in this app.
